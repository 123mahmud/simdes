		...
		<div class="row">
			...
			<div class="col-12">
				...
				<div class="tab-content">
					...
					@include('penjualan.penjualantanpaorder.tab_laporanpenjualan')
...
<script type="text/javascript">
	...
	function DetailPenjualan(id)
	{
		...
		$.ajax({
			url: baseUrl + "/penjualan/penjualantanpaorder/getDetailPenjualan/" + id,
			type: 'get',
			success : function (response){
	...
	function EditPenjualan(id)
	{
		...
		window.location.href = baseUrl + '/penjualan/penjualantanpaorder/edit/' + id;
	}
...
<script type="text/javascript">
	...
	function TableLaporanPenjualan()
	{
		...
		tb_laporanpenjualan = $('#table_laporanpenjualan').DataTable({
			...
			serverSide: true,
			ajax: {
				url: "{{ route('penjualantanpaorder.getlaporanpenjualan') }}",
				type: "get",
				data: {
					"_token": "{{ csrf_token() }}",
